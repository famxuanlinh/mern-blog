...
export const updateUser = async (req, res, next) => {
  ...
  if (req.user.username) {
    if (req.user.username < 7 || req.user.username > 20) {
      return next(
    ...
    if (req.user.username.match(/^[a-zA-Z0-9]+$/)) {
      return next(
        errorHandler(400, "Username must be only letters and numbers")
      );
    }
  }
  try {
...
export const getUsers = async (req, res, next) => {
  console.log("req getUsers", req.user);
  if (!req.user.isAdmin) {
